...
const TrafficSourcesSection = () => {
  ...
  return (
    <section className="min-h-screen w-full relative font-sans overflow-hidden text-white flex items-center bg-[#1a0b2e]">
      {/* Background Image */}
      <div
        className="absolute inset-0 bg-no-repeat bg-cover bg-center md:bg-right z-0 opacity-40 md:opacity-80"
      ...
      <div className="relative z-10 w-full h-full flex items-center pointer-events-none py-20">
        ...
        {/* Left Side Content */}
        <div className="w-full pointer-events-auto px-6 sm:px-8 md:px-0 md:pl-32 lg:pl-48 xl:pl-56 md:max-w-[850px] lg:max-w-[950px] xl:max-w-[1050px]">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-[3rem] font-gilroy font-bold leading-[1.1] sm:leading-[1.1] tracking-tight mb-8 sm:mb-10 md:mb-14">
            Why Partner <span className="text-[#a4d64f]">With Us?</span>
          </h2>

          {/* Reasons Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 sm:gap-y-8 md:gap-y-10">
            {reasons.map((r, index) => (
              <div
                key={index}
              ...
              >
                ...
                <h4 className="font-gilroy font-bold text-lg sm:text-xl md:text-2xl text-white group-hover:text-[#a4d64f] transition-colors mb-2">
                  {r.title}
                </h4>
